        ...
        with open(blast_result_file, "rt") as blast_result:
            ...
            for line in blast_reader:
                ...
                if gi in gi_taxid_taxonomy_dict:
                    annotation = gi_taxid_taxonomy_dict[gi]
                else:
                    annotation = None
                if line[0] in blast_dict:
                    blast_dict[line[0]] += [[annotation,
                                             float(line[4]), int(line[3]),
                                             round(100.0 * 
                                                   (float(line[3]) / 
                                                    float(line[2])),
                                                   2), float(line[5])]]
...
def write_annotation(blast_dict, nbest, filter_coverage, output_file, results):
    ...
    try:
        with open(output_file, "wt") as output:
            ...
            output_writer.writerow(["GeneName", "superkingdom", "kingdom",
                                    "phylum", "class", "order", "family",
                                    "genus","species", "PourcID", "nAlign",
                                    "Coverage", "evalue"])
            for key in blast_dict:
                ...
                data.sort(key=lambda x: x[2] + x[4], reverse=True)
                if nbest > 0:
                ...
                for hit in short_set:
                    ...
                    if hit[0]:
                        if hit[1] >= 95.0:
                            ...
                            hit[0] = hit[0].split(";")[0:4] + ["NA"] * 4
                        elif hit[1] >= 65.0:
                            hit[0] = hit[0].split(";")[0:3] + ["NA"] * 5
                        elif hit[1] > 0.0:
                            hit[0] = hit[0].split(";")[0:2] + ["NA"] * 6
                    else:
                        hit[0]= ["NA"] * 8
                for element in short_set:
                    output_writer.writerow([key] + element[0] + element[1:])
    except IOError:
