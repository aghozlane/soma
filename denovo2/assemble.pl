...
my $exeVELVET = "${exeDeNovo}/velvet_1.2.10";
...
while ($#ARGV >= 0) {
		...
		if ($opt =~ /^\-/) {
				if ($opt eq "-outdir") {
						...
						my $v = shift @ARGV;
						$min_pair_count = "-min_pair_count $v";
        }                  
...
if($NO_CORRECTION==0 && $ASSEMBLE_ONLY == 0)
{
 ...
 if($r3 ne ""){
  ...
  if($datareduced eq "1")
  {
   $res = system("cat $outdir/subreads2.csfasta >> $outdir/subreads.csfasta;rm -f $outdir/subreads2.csfasta;");
  } else
  {
   $res = system("mkdir -p $outdir;cp -f $reads_file $outdir/subreads.csfasta; cat $r3 >> $outdir/subreads.csfasta");
  }   
 if ($res != 0) {
 ...
 if($r3qv ne ""){
  ...
  if($datareduced eq "1")
  {
   $res = system("cat $outdir/subreads2.qual >> $outdir/subreads.qual; rm -f $outdir/subreads2.qual");
  } else
  {
   if($qual_file eq "none") {print "ERROR: First qual file is missing \n"; exit(1);}
   $res = system("mkdir -p $outdir;cp -f $qual_file $outdir/subreads.qual; cat $r3qv >> $outdir/subreads.qual");
  }   
 if ($res != 0) {
  print "Merging files Failed: $res\n";
  exit(1);
 ...
 my $res = system($errorCorrectReadsCmd);   
 if ($res != 0) {
...
if($ASSEMBLE_ONLY == 0){
 ...
 if($datareduced eq "1"){
  if($NO_CORRECTION==0){
   ...
   if($NO_CORRECTION==0){
   if($run_type eq "fragment"){
     my $debugfile = basename($reads_file);
     ...
     $preprocessingCmd = "${exeUTILS}/solid_denovo_preprocessor_v1.2.pl  --run_type $run_type --output $outdir/preprocessor --f3_file $outdir/fixed/$debugfile;";
   } else
   {
     $preprocessingCmd = "${exeUTILS}/solid_denovo_preprocessor_v1.2.pl  --run_type $run_type --output $outdir/preprocessor --mixed_tag_file $outdir/fixed/subreads.csfasta;"; 
   }
 ...
 my $res = system($preprocessingCmd);   
 if ($res != 0) {
...
if($min_contig_lgth eq ""){
		   #MATH $min_asid = ceil(2.0*($read_length));
...
$assembleGlobalScaffoldsCmd = 
 ...
 my $res = system($assembleGlobalScaffoldsCmd);   
 if ($res != 0) {
...
#*******************************************************************
{
 my $res = system("mkdir -p $outdir/postprocessor;mkdir -p $outdir/postprocessor/gap_reads;");   
 if ($res != 0) {
...
if($NO_CNT_MERGE == 0 && $run_type ne "fragment")
{
 ...
 my $res = system($extractGapReadsCmd);   
 if ($res != 0) {
...
my $res = system($assembleGapsCmd1);   
 if ($res != 0) {
 ...
 my $res = system($assembleGapsCmd2);   
 if ($res != 0) {
  print "Contigs merging failed when assembling local reads.\n";
  exit(1);
}
 ...
 my $res = system($assembleGapsCmd3);   
 if ($res != 0) {
 print "Contigs merging failed when assembling local reads.\n";
 exit(1);
}
 ...
 my $res = system($assembleGapsCmd4);   
 if ($res != 0) {
 print "Contigs merging failed when assembling local reads.\n";
 exit(1);
}
 ...
 { 
  ...
  my $res = system($convertMAtoBaseCmd);   
 if ($res != 0) {
  ...
  my $res = system($mergeBaseContigsCmd);   
 if ($res != 0) {
...
system("cp $outdir/velvet/contigs.fa $outdir/contigs.de");
...
if($NO_ANALYSIS == 0)
 ...
 if($run_type ne "fragment"){
    ...
    if (-e "$outdir/nt_scaffolds.fa")
    {
     ...
     my $res = system($mergescaffoldCmd);   
