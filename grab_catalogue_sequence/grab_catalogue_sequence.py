        ...
        with open(output_file, "wt") as output:
            for key in interest_sequence:
                output.write(">{1}{0}{2}{0}".format(
                                os.linesep, key,
                                "".join(interest_sequence[key])))
    except IOError:
        sys.exit("Error cannot open {0}".format(output_file))

#def write_interest_sequence_low_memory(list_sequences, catalogue_file, not_in_database, output_file):
    #"""
    #"""
    #interest_sequence = {}
    #try:
         ...
         #with open(output_file, "wt") as output:
           #for line in catalogue:
                    ...
                    #selection = get_element(title, list_sequences)
                        ...
                        #grab_sequence = True
                #elif grab_sequence and len(line) > 0:
                    #output.write(">{1}{0}{2}{0}".format(os.linesep, 
                                                             #title, 
                                                             #"".join(interest_sequence[title])))
    #except IOError:
        #sys.exit("Error cannot open {0} or {1}".format(catalogue_file, output_file))
...
def main():
    ...
    #write_interest_sequence_low_memory(list_sequences, args.catalogue_file,
                                       #args.not_in_database, args.output_file)
    print("Done.")
